#!/usr/bin/env bash
# Add (or update) the CNAME for kaliyai.castalia.institute → castaliainstitute.github.io
...
if echo "$OUT" | jq -e '.success' >/dev/null; then
  ...
  echo "Then GitHub → CastaliaInstitute/Kaliyai → Settings → Pages → Custom domain: kaliyai.castalia.institute"
else
  echo "API error:" >&2
  echo "$OUT" | jq . >&2
  exit 1
